...
// ------------------------------------------------------------
bool CostEvaluation::CheckCollision(double x0, double y0, double theta0, double x1, double y1, double theta1)
{
    Vector2d trans0;
    trans0 << x0, y0;
    Vector2d trans1;
...
int CostEvaluation::CheckCollisionOnTrajectory(TrajectoryXY const &trajectory, std::map<int, vector<Coord> > &predictions)
{
    ...
    while(it != predictions.end())
    {
        ...
        for (int i = 0; i < PARAM_MAX_COLLISION_STEP; i++)
        {
            // up to 50 (x,y) coordinates
...
double CostEvaluation::GetPredictedMinDistance(TrajectoryXY const &trajectory, std::map<int, vector<Coord> > &predictions)
{
    double dmin = INF;
...
void CostEvaluation::SetCost(double cost)
{
    m_dCost = cost;
}
